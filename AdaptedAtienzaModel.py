...
class face_DCGAN(object):
    ...
    def train(self, train_steps=2000, batch_size=256, save_interval=0):
        ...
        for i in range(train_steps):
            ...
            #eval_loss, eval_acc = self.discriminator.evaluate(x, y, batch_size=batch_size)
    ...
    def plot_images(self, save2file=False, fake=True, samples=16, noise=None, step=0):
        filename = 'testrun.png'
        if fake:
            if noise is None:
                noise = np.random.uniform(-1.0, 1.0, size=[samples, 100])
            else:
                filename = "testrun_%d.png" % step
...
def main():
    ...
    #Creates model and times
    face_dcgan = face_DCGAN()
    face_dcgan.x_train = x.astype(np.float32)
    timer = ElapsedTimer()
    ...
    ##Plots Graphs
    face_dcgan.plot_images(fake=True, save2file=True)
    plot_accuracy(d_acc_history,a_acc_history)
    plot_loss(d_acc_history,a_acc_history)
